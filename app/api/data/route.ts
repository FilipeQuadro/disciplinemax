...
export async function POST(req: Request) {
  ...
  if (!checkRateLimit(user.id)) return json({ error: "Rate limit exceeded" }, 429);

  try {
    const { action, table, filters, data: payload, id } = body;

    if (!table || !ALLOWED_TABLES.includes(table)) return json({ error: "Table not allowed" }, 403);

    const sb = getAdminClient();

    if (action === "select") {
      let query = sb.from(table).select(filters?.select || "*");
      if (filters?.eq) { for (const [k, v] of Object.entries(filters.eq)) query = query.eq(k, v as string); }
      if (filters?.gte) { for (const [k, v] of Object.entries(filters.gte)) query = query.gte(k, v as string); }
    ...
    if (action === "insert") {
      if (payload && !payload.user_id && table !== "admin_users") payload.user_id = user.id;
      if (payload?.user_id && payload.user_id !== user.id && table !== "admin_users") return json({ error: "User mismatch" }, 403);
    ...
    if (action === "update") {
      if (table !== "admin_users") {
        ...
        if (row && row.user_id !== user.id) return json({ error: "Not yours" }, 403);
      }
    ...
    if (action === "upsert") {
      if (!payload.user_id && table !== "admin_users") payload.user_id = user.id;
      if (payload?.user_id && payload.user_id !== user.id && table !== "admin_users") return json({ error: "User mismatch" }, 403);
