  ...
  const loadProfile = useCallback(async () => {
    ...
    try {
      const [profileRes, referralRes] = await Promise.all([
        authFetch(`/api/profile?userId=${user.id}`).then((r) => r.json()).catch(() => null),
        authFetch("/api/referral", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ userId: user.id, action: "get_code" }) }).then((r) => r.json()).catch(() => null),
      ]);
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-between p-3 rounded-xl card">
            <div>
              ...
              <p className="text-xs" style={{ color: "var(--text-secondary)" }}>Outros podem ver seu progresso em /u/{profileForm.username || "..."}</p>
            </div>
            <button
