...
export function ConfirmDialog({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center" onClick={onCancel}>
      ...
      >
        <button onClick={onCancel} className="absolute top-4 right-4 min-w-[44px] min-h-[44px] flex items-center justify-center rounded-lg hover:bg-white/5 transition-colors" style={{ color: "var(--text-secondary)" }} aria-label="Fechar diálogo">
          <X size={16} />
        </button>
        <div className="flex items-start gap-3 mb-4">
          ...
          <div>
            <h3 id="confirm-dialog-title" className="font-semibold tracking-tight text-white text-sm">{title}</h3>
